...
const arr_3 = [
  {
    xclip: {
      os: { name: "linux", command: "provided_installation_command_for_linux" }
    }
  },
  {
    pbcopy: {
...
describe("Dependencies Parser", () => {
  it("Standalone dependencies", () => {
    const ret_linux = parse_depencencies(arr_1, run_cmd_linux, os_name_linux);
  ...
  it("OS specific installation without provided installation command/ Linux", () => {
    const ret_darwin = parse_depencencies(arr_2, run_cmd_linux, os_name_linux);
    expect(ret_darwin).to.have.lengthOf(1);
  ...
  it("OS specific installation with provided installation command/ Linux", () => {
    const ret_linux = parse_depencencies(arr_3, run_cmd_linux, os_name_linux);
  ...
  it("Array of dependencies", () => {
    const ret_linux = parse_depencencies(arr_4, run_cmd_linux, os_name_linux);
