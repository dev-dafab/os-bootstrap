...
const createNewModule = require('./new-module')
...
function main () {
  program
    ...
    .action((files, { ignore, output }) => {
      ...
      files.forEach((file) =>
        runIn(_packages, (dir) => exec(`cp -f ${file} ${dir}/${_output}`))
      )
    })
  ...
  program
    ...
    .action((files, { ignore }) => {
      const _packages = getModules(ignore)
      files.forEach((file) =>
        runIn(_packages, (dir) => exec(`rm -f ${dir}/${file}`))
      )
    })
  ...
  program
    ...
    .action((packages, { dev, ignore }) => {
      const installationCommand = `yarn add ${packages.join(' ')} ${
        dev ? '-D' : ''
      }`
      runIn(getModules(ignore), (dir) =>
        exec(`cd ${dir} && ${installationCommand}`)
      )
    })

  program
    .command('lintFixPackageJson [packages...]')
    .option('--ignore <files>', 'depedencies to ignores', [])
  ...
  program
    .command('update-package-json [packages] <value>')
    .option('--ignore <files>', 'depedencies to ignores', [])
    .action((packages, value, { ignore }) => {
      const _packages =
        packages && packages.length > 0
          ? resolvePackages(packages.split(','), ignore)
          : getModules(ignore)
      updateOrAddEntry(_packages, value)
    })

  program
    .command('run <cmd> [packages...]')
    .option('--ignore <files>', 'depedencies to ignores', [])
  ...
  program.command('new-module <moduleName>').action((moduleName, { args }) => {
    console.log(args)
